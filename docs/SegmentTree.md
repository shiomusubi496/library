...
- コンストラクタ
  - `SegmentTree<T>(T op(T, T), T e)` : 二項演算 `op` と単位元 `e` で長さ $0$ に SegmentTree を初期化する。 $O(N)$ 。
  - `SegmentTree<T>(int n, T op(T, T), T e)` : 長さ `n` の SegmentTree を作成する。初期値は `e` 。 $O(N)$ 。
  - `SegmentTree<T>(vector<T> v, T op(T, T), T e)` : 列 `v` で SegmentTree を作成する。 $O(N)$ 。
  - `void init(vector<T> v)` : 列 `v` で SegmentTree を作成する。 $O(N)$ 。
- 変更クエリ
