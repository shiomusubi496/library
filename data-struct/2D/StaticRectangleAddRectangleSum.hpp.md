...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 2 \"data-struct/2D/StaticRectangleAddRectangleSum.hpp\"\n\n\
  ...
  code: "#pragma once\n\n#include \"../../other/template.hpp\"\n#include \"../../other/monoid2.hpp\"\
    ...
    \    const std::vector<std::pair<std::array<ll, 4>, T>>& A,\n    const std::vector<std::array<ll,\
    \ 4>>& B) {\n    int N = A.size(), M = B.size();\n    if (N == 0 || M == 0) return\
    \ std::vector<T>(M, 0);\n    compressor<ll> ps;\n    ps.reserve(2 * N + 2 * M);\n\
    \    rep (i, N) {\n        ps.push_back(A[i].first[0]);\n        ps.push_back(A[i].first[2]);\n\
    \    }\n    rep (i, M) {\n        ps.push_back(B[i][0]);\n        ps.push_back(B[i][2]);\n\
    \    }\n    ps.build();\n    std::vector<std::tuple<ll, ll, ll, ll, T>> C(2 *\
    \ N + 2 * M);\n    rep (i, N) {\n        ll l = ps.get(A[i].first[0]), r = ps.get(A[i].first[2]);\n\
    \        C[2 * i] = {A[i].first[1], M, l, r, A[i].second};\n        C[2 * i +\
    \ 1] = {A[i].first[3], M, l, r, -A[i].second};\n    }\n    rep (i, M) {\n    \
    \    ll l = ps.get(B[i][0]), r = ps.get(B[i][2]);\n        C[2 * N + 2 * i] =\
...
documentation_of: data-struct/2D/StaticRectangleAddRectangleSum.hpp
layout: document
redirect_from:
- /library/data-struct/2D/StaticRectangleAddRectangleSum.hpp
- /library/data-struct/2D/StaticRectangleAddRectangleSum.hpp.html
