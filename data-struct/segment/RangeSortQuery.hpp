...
private:
    ...
    public:
        ...
        static value_type op(const value_type& a, const value_type& b) {
            return {{M::op(a.first[0], b.first[0]), M::op(b.first[1], a.first[1])}, a.second + b.second};
        }
    };
    int n;
    ll mx;
    ...
    void split_l(int k) {
        ...
        if (l == k) return;
        if (!flag[l]) {
            ll t = seg[l].max_right(0, [&](const auto& x) { return x.second <= k - l; });
...
public:
    ...
    RangeSortQuery(const std::vector<T>& v, const std::vector<ll>& key,
                   ll mx = inf)
        : n(v.size()), mx(mx), seg(n, DynamicSegmentTree<M2>(mx + 1)), flag(n, false), dat(n), bit(n, 1) {
        rep (i, n) {
    ...
    void set(int k, ll key, const T& val) {
        ...
        if (!flag[l]) {
            ...
            ll t = seg[l].min_left(mx, [&](const auto& x) { return x.second <= k - l; }) - 1;
            if (l != k) {
    ...
    T get(int k) const {
        ...
        if (!flag[l]) {
            ...
            t = seg[l].min_left(mx, [&](const auto& x) { return x.second <= k - l; }) - 1;
        }
    ...
    T prod(int l, int r) {
        ...
        if (l2 == r2) {
            if (!flag[l2]) {
                ll tl = seg[l2].max_right(0, [&](const auto& x) { return x.second <= l - l2; });
                ll tr = seg[l2].max_right(0, [&](const auto& x) { return x.second <= r - l2; });
                return seg[l2].prod(tl, tr).first[0];
            }
            else {
                ll tl = seg[l2].min_left(mx, [&](const auto& x) { return x.second <= l - l2; });
                ll tr = seg[l2].min_left(mx, [&](const auto& x) { return x.second <= r - l2; });
                return seg[l2].prod(tr, tl).first[1];
            }
        }
        if (!flag[l2]) {
            ll t = seg[l2].max_right(0, [&](const auto& x) { return x.second <= l - l2; });
            res = seg[l2].prod(t, mx + 1).first[0];
        }
        else {
            ll t = seg[l2].min_left(mx, [&](const auto& x) { return x.second <= l - l2; }) - 1;
            res = seg[l2].prod(0, t + 1).first[1];
        }
        res = M::op(res, dat.prod(l2 + 1, r2));
        if (!flag[r2]) {
            ll t = seg[r2].max_right(0, [&](const auto& x) { return x.second <= r - r2; });
            res = M::op(res, seg[r2].prod(0, t).first[0]);
        }
        else {
            ll t = seg[r2].min_left(mx, [&](const auto& x) { return x.second <= r - r2; }) - 1;
