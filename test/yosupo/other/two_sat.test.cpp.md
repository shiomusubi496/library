...
data:
  ...
  bundledCode: "#line 1 \"test/yosupo/other/two_sat.test.cpp\"\n#define PROBLEM \"\
    ...
    \      cmp.assign(n, -1);\n        st.reserve(n);\n        rep (i, n) {\n    \
    \        if (ord[i] == -1) dfs(i);\n        }\n        for (auto&& i : cmp) i\
    \ = sz - i - 1;\n    }\n\npublic:\n    StronglyConnectedComponents(const Graph<T>&\
    \ G) : G(G) { init(); }\n    StronglyConnectedComponents(Graph<T>&& G) : G_(std::move(G)),\
    ...
    \ add_if(int a, int b) { add_clause(a, false, b, true); }\n    void add_nand(int\
    \ a, int b) { add_clause(a, false, b, false); }\n    void add_eq(int a, int b)\
  ...
  verifiedWith: []
documentation_of: test/yosupo/other/two_sat.test.cpp
layout: document
redirect_from:
- /verify/test/yosupo/other/two_sat.test.cpp
- /verify/test/yosupo/other/two_sat.test.cpp.html
