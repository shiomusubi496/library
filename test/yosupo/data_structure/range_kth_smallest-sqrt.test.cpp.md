...
data:
  ...
  bundledCode: "#line 1 \"test/yosupo/data_structure/range_kth_smallest-sqrt.test.cpp\"\
    ...
    \ docs/data-struct/segment/SqrtDecomposition.md\n */\n#line 5 \"test/yosupo/data_structure/range_kth_smallest-sqrt.test.cpp\"\
    \nusing namespace std;\nint main() {\n    int N, Q; scan >> N >> Q;\n    vector<int>\
    \ A(N); scan >> A;\n    compressor<int> ps(A, true);\n    ps.press(A);\n    auto\
    \ f = [&](vector<int>&& v) -> vector<int> {\n        vector<int> A(ps.size());\n\
    \        for (const auto& i : v) ++A[i];\n        rep (i, ps.size() - 1) A[i +\
    \ 1] += A[i];\n        return A;\n    };\n    SqrtDecomposition<int, decltype(f)>\
