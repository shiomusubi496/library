...
data:
  ...
  bundledCode: "#line 1 \"test/atcoder/abc349_g-RangeUF.test.cpp\"\n#define PROBLEM\
    ...
    \ */\n#line 4 \"test/atcoder/abc349_g-RangeUF.test.cpp\"\nusing namespace std;\n\
    \nvector<int> solve_static(int N, vector<int> A) {\n    vector<array<int, 3>>\
    \ qs(N);\n    rep (i, N) qs[i] = {(int)i - A[i], 2 * N - (int)i - 1 - A[i], 2\
    ...
    \        while (st.count(cnt)) ++cnt;\n        for (int j : grp[uf.find(i)]) res[j]\
    \ = cnt;\n    }\n    res.resize(N);\n    return res;\n}\n\nvector<int> solve_dynamic(int\
    \ N, vector<int> A) {\n    RangeParallelUnionFind ruf(2 * N);\n    rep (i, N)\
    \ ruf.merge(i - A[i], 2 * N - 1 - i - A[i], 2 * A[i] + 1);\n    auto uf = ruf.get_uf();\n\
    \    rep (i, N) uf.merge(i, 2 * N - 1 - i);\n    vector<vector<int>> g(2 * N);\n\
    \    rep (i, N) {\n        if (i - A[i] == 0 || i + A[i] == N - 1) continue;\n\
    \        int a = uf.find(i - A[i] - 1), b = uf.find(i + A[i] + 1);\n        if\
    \ (a == b) return {};\n        g[a].push_back(b);\n        g[b].push_back(a);\n\
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/abc349/tasks/abc349_g\"\n#include\
    ...
    \nusing namespace std;\n\nvector<int> solve_static(int N, vector<int> A) {\n \
    \   vector<array<int, 3>> qs(N);\n    rep (i, N) qs[i] = {(int)i - A[i], 2 * N\
    \ - (int)i - 1 - A[i], 2 * A[i] + 1};\n    auto uf = static_range_parallel_union_find(2\
    \ * N, qs);\n    rep (i, N) uf.merge(i, 2 * N - 1 - i);\n    vector<vector<int>>\
  ...
  verifiedWith: []
documentation_of: test/atcoder/abc349_g-RangeUF.test.cpp
layout: document
redirect_from:
- /verify/test/atcoder/abc349_g-RangeUF.test.cpp
- /verify/test/atcoder/abc349_g-RangeUF.test.cpp.html
