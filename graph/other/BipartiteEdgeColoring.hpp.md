...
data:
  ...
  code: "#pragma once\n\n#include \"../../other/template.hpp\"\n#include \"../../data-struct/unionfind/UnionFind.hpp\"\
    ...
    \ T> class BipartiteEdgeColoring {\nprivate:\n    int color_number;\n    std::vector<int>\
    \ edge_color;\n\n    template<class U> std::vector<int> get_trails(int N, const\
    \ Edges<U>& es) {\n        UnionFind uf(2 * N);\n        for (const auto& e :\
    \ es) uf.merge(e.from, N + e.to);\n        std::vector<int> res;\n        std::vector<int>\
    \ idx(2 * N, -1);\n        std::vector<Graph<int>> G(2 * N);\n        rep (i,\
    \ 2 * N) {\n            if (uf.is_root(i)) G[i].reserve(uf.size(i));\n       \
    \ }\n        rep (i, 2 * N) {\n            int r = uf.find(i);\n            idx[i]\
    \ = G[r].size();\n            G[r].emplace_back();\n        }\n        rep (i,\
...
layout: document
redirect_from:
- /library/graph/other/BipartiteEdgeColoring.hpp
- /library/graph/other/BipartiteEdgeColoring.hpp.html
