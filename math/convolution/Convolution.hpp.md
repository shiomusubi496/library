...
data:
  ...
  code: "#pragma once\n\n#include \"../../other/template.hpp\"\n#include \"../ModInt.hpp\"\
    ...
    \ = mod_pow(primitive_root_for_convolution(p), (p - 1) >> lg, p);\n        inv_root[lg]\
    \ = mod_pow(root[lg], p - 2, p);\n        rrep (i, lg) {\n            root[i]\
    \ = (ull)root[i + 1] * root[i + 1] % p;\n            inv_root[i] = (ull)inv_root[i\
    ...
    \ m = b.size();\n    std::vector<T> c(n + m - 1);\n    rep (i, n)\n        rep\
    \ (j, m) c[i + j] += a[i] * b[j];\n    return c;\n}\n\n} // namespace internal\n\
    \ntemplate<unsigned int p>\nstd::vector<static_modint<p>>\nconvolution_for_any_mod(const\
    \ std::vector<static_modint<p>>& a,\n            const std::vector<static_modint<p>>&\
    \ b);\n\ntemplate<unsigned int p>\nstd::vector<static_modint<p>>\nconvolution(const\
    \ std::vector<static_modint<p>>& a,\n            const std::vector<static_modint<p>>&\
    \ b) {\n    unsigned int n = a.size(), m = b.size();\n    if (n == 0 || m == 0)\
    \ return {};\n    if (n <= 60 || m <= 60) return internal::convolution_naive(a,\
