...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 2 \"math/matrix/BlackboxLinearAlgebra.hpp\"\n\n#line 2 \"other/template.hpp\"\
    ...
    \ * @brief Random\n * @docs docs/random/Random.md\n */\n#line 2 \"math/poly/BerlekampMassey.hpp\"\
    ...
    #line 5 \"math/poly/BerlekampMassey.hpp\"\n\ntemplate<class T>\nFormalPowerSeries<T>\
    \ berlekamp_massey(std::vector<T> S) {\n    int N = S.size();\n    FormalPowerSeries<T>\
    \ Q{1}, B{1};\n    int L = 0, p = -1;\n    T b = 1;\n    rep (i, N) {\n      \
    \  T d = 0;\n        rep (j, Q.size()) d += Q[j] * S[i - j];\n        if (d ==\
    \ 0) continue;\n        FormalPowerSeries<T> R = Q - ((B * d / b) << (i - p));\n\
    \        if (2 * L <= i) {\n            L = i + 1 - L;\n            B = std::move(Q);\n\
    \            p = i;\n            b = d;\n        }\n        Q = std::move(R);\n\
    \    }\n    return Q.prefix(L + 1);\n}\n#line 2 \"math/poly/ConsecutiveTermsOfLinearRecurrence.hpp\"\
    \n\n#line 5 \"math/poly/ConsecutiveTermsOfLinearRecurrence.hpp\"\n\ntemplate<class\
    ...
    \ b) {\n        rep (i, n) b[i] *= D[i];\n        return A(b);\n    });\n    T\
    \ detd = 1;\n    rep (i, n) detd *= D[i];\n    return chr[0] / detd * (n & 1 ?\
    \ -1 : 1);\n}\n\ntemplate<class T, class F> std::vector<T> matrix_pow_blackbox(int\
    \ n, F&& A, ll K, std::vector<T> b) {\n    std::vector<T> a(2 * n), c(n);\n  \
    \  std::vector<std::vector<T>> bs(2 * n);\n    bs[0] = b;\n    rep (i, n) c[i]\
    \ = rand32.uniform<unsigned int>(1, T::get_mod() - 1);\n    rep (i, 2 * n) {\n\
    \        rep (j, n) a[i] += bs[i][j] * c[j];\n        if (i < 2 * n - 1) bs[i\
    \ + 1] = A(bs[i]);\n    }\n    FormalPowerSeries<T> r = x_pow_k_mod_F(berlekamp_massey(a).rev(),\
    \ K);\n    std::vector<T> d(n);\n    rep (i, r.size()) {\n        rep (j, n) d[j]\
    \ += bs[i][j] * r[i];\n    }\n    return d;\n}\n"
...
documentation_of: math/matrix/BlackboxLinearAlgebra.hpp
layout: document
redirect_from:
- /library/math/matrix/BlackboxLinearAlgebra.hpp
- /library/math/matrix/BlackboxLinearAlgebra.hpp.html
